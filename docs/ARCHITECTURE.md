...
│                                                                                  │
│  ┌────────────────────────────────────────────────────────────────────────────┐ │
│  │ Pipeline Anti-Alucinación v8.0 (3 Capas)                                   │ │
│  │                                                                             │ │
...
│                                                                                  │
│  ┌────────────────────────────────────────────────────────────────────────────┐ │
│  │ Algoritmo Multicriteria v8.1                                               │ │
│  │                                                                             │ │
│  │  PASO 1: Título (50%)       →  BGE-M3 + ESCO-XLM re-ranking               │ │
│  │  PASO 2: Skills (40%)       →  SQL lookup en esco_associations             │ │
│  │  PASO 3: Descripción (10%)  →  BGE-M3 embeddings                          │ │
│  │  PASO 4: Ajustes v8.3       →  Familias funcionales + never_confirm        │ │
│  └────────────────────────────────────────────────────────────────────────────┘ │
│                                    │                                             │
│                                    ▼                                             │
...
├── prompts/
│   └── extraction_prompt_v8.py         # CAPA 1: Prompt ultra-conservador
└── (pipeline ejecuta desde database/)

database/
└── process_nlp_from_db_v7.py           # Pipeline principal v8.0
```
...
#### Outputs
- Tabla `ofertas_nlp` (26 campos)
- Tabla `validacion_v7` (datos por capa para validación humana)
...
```
database/
├── match_ofertas_multicriteria.py      # Algoritmo v8.1 (4 pasos)
├── matching_rules_v83.py               # ACTIVO: Reglas de ajuste
├── matching_rules_v82.py               # Anterior
├── matching_rules_v81.py               # Anterior
└── embeddings/
    ├── esco_occupations_embeddings.npy # Embeddings de ocupaciones
    └── esco_occupations_metadata.json  # Metadata de ocupaciones
```
...
CREATE TABLE ofertas_esco_matching (
    ...
    -- Metadata legacy
    occupation_match_method  TEXT,        -- "v8.1_multicriterio_validado"
    ...
    matching_version         TEXT,        -- "v8.1_esco_multicriterio_validado"
    confidence_score         REAL
);
```

### 3.4 Formato: `gold_set_manual_v1.json`
...
python process_nlp_from_db_v7.py --mode production --limit 500

# 4. ESCO MATCHING - Clasificar ocupaciones
python match_ofertas_multicriteria.py
...
```bash
python database/process_nlp_from_db_v7.py --mode test --ids 2163782 2154549 --verbose
```
...
- extraction_prompt_v8.py   → Prompt LLM, versión 8
- matching_rules_v83.py     → Reglas ESCO, versión 8.3
- process_nlp_from_db_v7.py → Pipeline NLP, versión 7 (usa prompt v8)
```
...
| **Prompt LLM** | `extraction_prompt_v8.py` | v7, v6 |
| **Pipeline NLP** | `process_nlp_from_db_v7.py` | v6 |
| **Reglas matching** | `matching_rules_v83.py` | v82, v81 |
| **Gold set** | `gold_set_manual_v1.json` | - |
...
```
======================================================================
VALIDACION GOLD SET MANUAL - MATCHING ESCO
======================================================================

[1] Gold set cargado: 19 casos
[2] Matches en DB: 19
...
```python
CRITERIOS = {
    "precision_minima": 50.0,      # Gold set test falla si < 50%
    ...
    "score_rango": (0.40, 0.95)    # Score final en rango razonable
}
```
