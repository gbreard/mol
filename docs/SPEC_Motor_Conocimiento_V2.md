...
- `scripts/db/create_chromadb_esco.py` línea 29: igual
- `config/matching_config.json`: agregar campo `"modelo_revision": "<SHA>"`
- VPS: actualizar el servidor FastAPI con el mismo SHA
...
def _verify_corpus_compatibility(self):
    ...
    # Obtener revision del modelo cargado
    actual_revision = self.model._modules['0'].auto_model.config._commit_hash
...
4. Correr Gold Set → si precisión < baseline - 2%, rollback automático
...
**Diseño:** El extractor de skills sigue buscando en las 14.247 skills ESCO. Cuando una skill aparece en `esco_argentino` para la ocupación que se está procesando, su score recibe un bonus proporcional a su frecuencia en el mercado argentino.

**Por qué boosting y no filtrado:** El perfil cubre solo 44 ocupaciones. Para las 3.001 restantes no hay información en `esco_argentino`. El filtrado dejaría esas ocupaciones sin ningún beneficio. El boosting degrada graciosamente: si no hay perfil argentino, el matching es idéntico al actual.

**Por qué proporcional a frecuencia:** Las skills con `frequency: 3` (aparecen en el 50% de las ofertas de esa ocupación en Argentina) deben subir más en el ranking que skills con `frequency: 1`. La frecuencia ya está en el JSONB de `skills_consolidadas`.

**Implementación en `skills_implicit_extractor.py`:**

```python
def _get_argentino_boost(self, occupation_uri: str) -> dict[str, float]:
    """
    Retorna dict {skill_uri: boost_factor} para la ocupación dada.
    boost_factor = 0.05 * (frequency / max_frequency_in_occupation)
    Si no hay perfil argentino para la ocupación, retorna {}.
    """
    if occupation_uri not in self._argentino_cache:
        # Cargar desde esco_argentino (Supabase o cache local)
        skills = self._load_argentino_skills(occupation_uri)
        if not skills:
            self._argentino_cache[occupation_uri] = {}
        else:
            max_freq = max(s.get('frequency', 1) for s in skills)
            self._argentino_cache[occupation_uri] = {
                s['esco_uri']: 0.05 * (s.get('frequency', 1) / max_freq)
                for s in skills
            }
    return self._argentino_cache[occupation_uri]

def extract_skills(self, texto: str, occupation_uri: str = None) -> list[dict]:
    # ... lógica existente ...
    
    # Aplicar boost argentino si hay ocupación
    if occupation_uri:
        boost_map = self._get_argentino_boost(occupation_uri)
        for result in results:
            if result['uri'] in boost_map:
                result['score'] = min(1.0, result['score'] + boost_map[result['uri']])
                result['boost_applied'] = True
    
    return sorted(results, key=lambda x: x['score'], reverse=True)
```

**Nota:** El boost máximo es 0.05 (5 puntos porcentuales). No puede llevar una skill de 0.35 a superar el threshold de 0.40 artificialmente — solo mejora el ranking de skills que ya superarían el threshold por mérito propio.
...
Meta: bajar de 7.026 sin CLAE a menos de 2.000 (las que genuinamente no tienen suficiente información).
