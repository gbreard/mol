...
|---------|-------|---------|
| 1.4 | 2026-04-08 | E1.6 nueva sub-etapa: equivalencias de skills vinculadas a embeddings. E1.5 agrega verificación de lookup post-regeneración |
| 1.3 | 2026-04-08 | Eliminar chromadb de tabla archivos. E2.5 proceso manual honesto. E3.3 throttling solo en sync. E2.2 punto integración match_and_persist(). E4.3 formato contrastivo unificado |
...
7. Verificar integridad del lookup de equivalencias:
   SELECT COUNT(*) FROM skill_equivalence_lookup sel
   LEFT JOIN skills_embeddings se ON sel.skill_uri = se.skill_uri
   WHERE se.skill_uri IS NULL
   → si > 0: reportar URIs huérfanas (no bloquea pero hay que revisar)
   (Ver E1.6 para contexto completo de equivalencias)

---

## E1.6 — Equivalencias de skills: vínculo con embeddings

**Contexto:**
El sistema tiene 1.091 grupos de equivalencias de skills generados con
AgglomerativeClustering sobre los mismos embeddings BGE-M3 que usa el
pipeline. Cada grupo agrupa variantes semánticas de la misma skill ESCO
(ej: "trabajar en equipo", "trabajo en equipo", "principios del trabajo
en equipo" → un solo grupo con representante canónico).

**Tablas:** `skill_equivalences`, `skill_equivalence_lookup`,
`ofertas_skills` (columnas `equivalence_id`, `canonical_label`)

**Estado actual:**
- 1.091 grupos, todos con estado='aprobado'
- La aprobación fue masiva (UPDATE sin revisión individual)
- Solo 5 tienen revisado_por documentado (Diego Schleser)
- El re-clustering congela los 'aprobado' → con todo aprobado,
  el re-clustering no tendría skills libres para re-agrupar

**Decisión de diseño para E1.5:**

```
Opción elegida: Opción A — conservadora
    Dejar los 1.091 grupos aprobados como están
    E1.5 NO incluye re-clustering de equivalencias
    El re-clustering solo aplica a skills nuevas
    que lleguen después del clustering original

Razón:
    Los embeddings actuales son la base de los grupos
    Degradar grupos sin revisión individual introduce riesgo
    Mejor esperar a que el Gold Set esté expandido (150+ casos)
    y los embeddings sean más confiables (post fine-tuning)
```

**Implicancia para E1.5:**
E1.5 regenera embeddings pero NO re-clusteriza equivalencias.
Después de E1.5, el pipeline sigue usando los 1.091 grupos
actuales. Las skills nuevas que no pertenezcan a ningún grupo
se tratan como singles (comportamiento actual, sin cambio).

**Mejora futura (no en este spec):**
Cuando el fine-tuning de BGE-M3 esté completo (E4.4),
degradar grupos con similitud_minima < 0.87 a estado='auto'
y correr re-clustering con los nuevos embeddings mejorados.
Ese es el momento correcto para revisar los grupos.

**Lo que sí hay que hacer en E1.5:**
Después de regenerar embeddings, verificar que el lookup
`skill_equivalence_lookup` sigue siendo válido — es decir,
que las URIs en los grupos siguen existiendo en el nuevo corpus.
Si alguna URI desapareció, el lookup queda con entradas huérfanas.

```python
# Verificación post-E1.5:
# SELECT COUNT(*) FROM skill_equivalence_lookup sel
# LEFT JOIN skills_embeddings se ON sel.skill_uri = se.skill_uri
# WHERE se.skill_uri IS NULL
# → debería ser 0
...
**Punto de integración exacto en match_ofertas_v3.py:**
El boost se aplica dentro de `match_and_persist()` (línea ~1537),
entre el match de ocupación y la persistencia de skills:

```python
# En match_and_persist(), después de determinar resultado_matching:
ocupacion_uri = resultado_matching.get('esco_uuid')
if ocupacion_uri and oferta_nlp.get('skills_implicitas'):
    oferta_nlp['skills_implicitas'] = extractor.rerank_with_argentino_boost(
        oferta_nlp['skills_implicitas'], ocupacion_uri
    )
# Luego save_matching_result() persiste las skills re-rankeadas
```

NO insertar en `oferta_nlp['skills_implicitas']` desde fuera del método
— las skills se persisten via `save_matching_result()` en la línea ~1538.
...
**Throttling:**
El reprocesamiento CLAE es SQLite local — no hay rate limit en esa etapa.
El throttling aplica solo al sync posterior a Supabase
(cuando se llama sync_to_supabase.py para subir los resultados).
El sync ya tiene su propio mecanismo de batch — no agregar throttling
adicional al script de reprocesamiento CLAE.
...
**Decisión de formato unificado (prerequisito de E4.3):**

Los pares existentes y los nuevos usan formatos distintos:
- 66 pares humanos + correcciones Cynthia: formato supervised
  `{input, clasificacion_incorrecta, clasificacion_correcta}`
- 291 pares de esco_argentino (E4.2): formato contrastivo
  `{query, positive, negatives[]}`

**Decisión: convertir TODO a formato contrastivo.**
Es el formato más rico y compatible con MultipleNegativesRankingLoss
de sentence-transformers, que es el loss recomendado para fine-tuning
de modelos de embeddings.

Conversión de pares supervised a contrastivo:
```python
# Par supervised existente:
# {input: "soldador", clasificacion_incorrecta: "8160", correcta: "7212"}
# →
# {query: "soldador",
#  positive: "soldador/soldadora (ESCO label 7212)",
#  negatives: ["operario de prensado de fruta (ESCO label 8160)"]}
```

El script `generate_training_pairs_from_argentino.py` (E4.2) genera
directamente en formato contrastivo. Los pares existentes se convierten
con un script one-time en E4.1.
...
| E1.6 Equivalencias | test que lookup no tiene URIs huérfanas post-regeneración |
...
- [ ] Post-regeneración: skill_equivalence_lookup sin URIs huérfanas
