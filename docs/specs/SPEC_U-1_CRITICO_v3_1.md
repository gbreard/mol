# SPEC U-1 — Crítico: datos producto-visibles (v3.1)

**Versión:** 3.1 (final, con 7 patches de auditoría aplicados)
**Fecha:** 2026-05-05
**Autor:** Gerardo + Claude
**Estado:** Listo para implementación
**Predecesores:** v1.0, v2.0, v3.0 (todas del 2026-05-04/05)
**Diagnóstico base:** SPEC U original + 9 reportes de diagnóstico R1-R9 + auditoría de v3.
...
- Auditoría v3 (Gerardo): 7 gaps detectados → patches aplicados en v3.1.

**Cambios v3 → v3.1 (7 patches de auditoría):**

| Patch | Gap | Sección | Cambio |
|---|---|---|---|
| P1 | Gap 7 | §4.1 | Test concreto: re-matchear 5 ofertas pre-C1 y verificar 19 columnas pobladas |
| P2 | Gap 1 | §5.6 sub-fase D | Las 3 ofertas residuales de "jefe de mantenimiento" se incluyen explícitamente |
| P3 | Gap 2 | §6.1.1 (nuevo) | Plan B para C3 si schema-mismatch (B1 regenerar / B2 adaptar matcher) |
| P4 | Gap 3 | §8.3 | Reescribir lógica del DELETE post-sync con definición operativa inequívoca + anti-patrones |
| P5 | Gap 4 | §5.7 | Cuantificar regresión esperada del contexto #11 eliminado (≤60 ofertas) |
| P6 | Gap 6 | §10.1 (nuevo) | F0b — re-snapshot Supabase pre-C5 |
| P7 | Gap 5 | §7.6 | Corregir cálculo: ~784K (no ~755K), explicar fórmula |
...
- Confirmar que el matcher actual (post-C2) escribe las 19 columnas correctamente. **Test concreto:** re-matchear 5 ofertas con `match_ofertas_v3.py` post-C2 y verificar que las 19 columnas listadas en R3 §E1 (`esco_occupation_uri`, `regla_aplicada`, `isco_label`, `isco_regla`, `isco_semantico`, `decision_razon`, `occupation_match_method`, `dual_coinciden`, y las 11 restantes) quedan poblada-y-coherentes con la nueva URI. Si alguna queda vacía o desincronizada con `esco_occupation_uri` → ABORTAR C1 hasta que se arregle el matcher.
...
**Reprocesamiento adicional para "jefe de mantenimiento" (decisión §5.5):**

R9 §C confirmó que de 149 ofertas con título "jefe de mantenimiento", **3 están actualmente en ISCO 1321** (las que el SPEC pretende reclasificar a 1219). Las otras 146 ya están en otros ISCOs distintos al matcher (no son afectadas por el cambio del JSON).

- Identificar las 3 ofertas: `SELECT id_oferta FROM ofertas_esco_matching WHERE isco_code = '1321' AND ofertas.titulo LIKE '%jefe de mantenimiento%';`
- Si esas 3 están entre las 8.221 de `spec_h_rematch`: C1 las cubre automáticamente. **Confirmar con query antes de C1.**
- Si NO están entre `spec_h_rematch`: agregar las 3 al reprocesamiento de sub-fase D explícitamente.
- Para las 35 ofertas de "analista de tesoreria" y 213 de "operador de atencion" (cambios de label, mismo ISCO): UPDATE directo de `esco_occupation_label` post-sub-fase D (no requiere re-matchear).
...
**Impacto cuantificado del contexto eliminado (R9 §A4 + auditoría tuya):**

- 20 ofertas con URI vacía matchean título "operador.{maquinas|cnc|torno}" → perderán cobertura del diccionario, caen al semántico/reglas.
- 43 ofertas en ISCO 8211 actualmente, 100% por path semántico (ninguna por regla). Sample auditado: "Operador de Torno", "Operador CNC", "Operador Plegado y Corte CNC" — todas mal-clasificadas hoy en 8211 (ESCO 8211 son montadores, no operadores).
- **Regresión semántica esperada: ≤60 ofertas afectadas.** Probablemente sigan siendo mal-clasificadas a 8211 (montadores) o caigan a otro ISCO incorrecto, equivalente a la situación actual.
- **Diferimiento explícito:** la decisión de re-introducir el contexto con ISCO correcto (probable 7223 o 8121) queda **fuera de alcance** de SPEC U-1, se difiere a fase 2 con datos. SPEC U-2 puede revisitar.
...
### 6.1.1 Plan B — Schema mismatch detectado

Si la validación previa revela que el schema enriched de SPEC E NO es compatible con `_load_occupation_embeddings()` o `_semantic_match_title()` actuales, escalar a Plan B:

**Opción B1 — Regenerar embeddings simples desde catálogo (~30 min en GPU):**
- Script para regenerar `esco_occupations_embeddings.npy` y `esco_occupations_metadata.json` con schema antiguo desde `esco_occupations` SQL local.
- Modelo: BGE-M3 con revision SHA pinneada (`5617a9f6...` según R5 §A1).
- 3.046 ocupaciones × 1024-dim = ~12 MB final.
- Validar contra hash conocido si existe.

**Opción B2 — Adaptar matcher al schema enriched (~2-3h):**
- Modificar `_load_occupation_embeddings()` para aceptar el schema enriched (con campos adicionales `label_es`, `L1`, `L2`, `broader`, `top_3_occupations`).
- Modificar `_semantic_match_title()` para usar nuevos campos si aplican.
- Tests sobre 100 ofertas.

**Decisión entre B1 y B2:** B1 es más rápido y conservador. B2 es preferible si el equipo decide aprovechar las features de SPEC E (broader, L1/L2 jerárquicos). Para SPEC U-1 → recomendar **B1** (mínima disrupción).

Si Plan B se activa, agregar +30 min a +3h al esfuerzo de C3.
...
- C-Q2 (filas con `is_essential=0 AND is_optional=0`) baja de 1.116.011 a aproximadamente **784.000**:
  - Las 92.100 filas no-backfilleables (URI vacía) siguen en cero
  - 67,6% de las 1.023.911 backfilleables siguen en cero (skill no en catálogo de su ocupación, legítimo)
  - Total esperado: 92.100 + (0,676 × 1.023.911) = ~784.263
...
### 8.3 Fix del re-sync para que purgue al sincronizar

**Atención: la redacción de este fix requiere precisión. La intención es purgar skills cuyas ofertas ya no son válidas, NO purgar todo lo que no esté en el batch del sync incremental.**

**Definición operativa correcta del DELETE post-sync:**

> Tras un sync (incremental o completo), eliminar de `ofertas_skills` (Supabase) todas las filas cuyo `id_oferta` cumpla AMBAS condiciones:
> 1. NO existe en `ofertas_dashboard` (Supabase) post-sync.
> 2. NO existe en el universo de ofertas validadas locales (`ofertas` con estado_validacion IN ('validado', 'validado_claude', 'validado_humano')).

Esto elimina solo skills genuinamente huérfanas (su oferta padre ya no existe en ningún lado del filtro de validación), no skills cuyas ofertas están pendientes de subirse.

**Pseudo-SQL del DELETE post-sync:**

```sql
-- En Supabase, después de cada sync exitoso
DELETE FROM ofertas_skills os
WHERE os.id_oferta NOT IN (SELECT id_oferta FROM ofertas_dashboard)
  AND os.id_oferta NOT IN (
    -- universo de validadas locales que el sync debió subir
    -- requiere que el script local exporte ese set como referencia
    SELECT id_oferta FROM ofertas_validadas_local_export
  );
```

**Implementación:**
- El script `sync_to_supabase.py` actual debe ser modificado para:
  1. Antes del sync: identificar el universo de validadas locales (set A).
  2. Hacer el sync incremental o completo.
  3. Post-sync: identificar zombies con la query de arriba (intersección de "no en ofertas_dashboard" Y "no en universo A").
  4. Si conteo de zombies < umbral seguro (500): ejecutar DELETE.
  5. Si conteo > umbral: PARAR, log de error, alertar.

**Test de regresión:**
- Caso 1: oferta X validada localmente, sus skills en Supabase, oferta X aún NO subida → DELETE no debe borrarla (universo A la incluye).
- Caso 2: oferta Y borrada localmente (estado=baja o invalidada), sus skills aún en Supabase → DELETE debe borrarlas.
- Caso 3: sync incremental con solo 100 ofertas modificadas → DELETE no debe borrar las skills de las 56.397 ofertas no-modificadas (universo A las incluye).

**Lo que NO se debe hacer (anti-patrón explícito):**

- ❌ NO usar `DELETE WHERE id_oferta NOT IN (batch_actual_del_sync)`. Eso borra TODO lo que no esté en el sync incremental = pérdida masiva.
- ❌ NO ejecutar el DELETE antes del sync inicial masivo (paso 1 de §8.2). El backlog de 40K ofertas hace que el universo de validadas locales no esté reflejado en Supabase aún.
...
F0 (snapshot + canarios)
 │
 ├──► C3 (embeddings ocupaciones — symlinks o Plan B si mismatch)
 │
 └──► C2 (fix diccionario híbrido)
       ...
       └─► Sub-fase D: reprocesamiento 3.758 ofertas + 3 ofertas residuales jefe mantenimiento
 │
 ▼
C1 (re-rematch URI×label — desvalida 8.210)
   └─ test concreto de 5 ofertas antes del re-rematch global
 │
 ▼
C4 (índice compuesto + UPDATE flags + medir F-meta)
 │
 ▼
F0b (re-snapshot Supabase pre-C5)
 │
 ▼
...
### 10.1 F0b — Re-snapshot Supabase pre-C5 (sub-paso nuevo)

**Justificación:** F0 toma snapshot Supabase en día 1. C5 corre en día 4. En esos 3-4 días, otros usuarios pueden modificar Supabase (Cynthia validando, issues nuevos). Si C5 falla y necesita rollback al snapshot F0 = pérdida de hasta 4 días de cambios.

**Acción:** justo antes de iniciar C5 (paso 1 sync forzado), tomar nuevo snapshot Supabase:
- Export JSON de `ofertas_dashboard`, `ofertas_skills`, `issues`, `rule_candidates`, `validacion_humana`.
- Path: `data/snapshots/pre_c5_supabase_YYYYMMDD_HHMMSS.json.gz`.
- Tiempo: ~5-10 min.

**Rollback de C5 desde F0b** (no desde F0): preserva los 3-4 días de cambios entre F0 y F0b.

### 10.2 Tiempos calendarios
...
- Día 4: C4 + F0b + C5 + verificación F-meta + buffer
