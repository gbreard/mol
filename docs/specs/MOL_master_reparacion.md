...
**P-14 PARCIALMENTE RESUELTO — primer cierre de loop del proyecto (2026-06-30)**: G3 cerró el loop de Cyn en su primera vía. Hasta aquí (b)=0 — ninguna corrección de Cyn volvía al sistema. Ahora **46 denominaciones argentinas validadas por Cyn clasifican ofertas** vía el diccionario, resolviendo por `esco_code` (el código que Cyn citó) a través de `code_to_occupation` — el mismo mecanismo que las reglas. El loop roto cerró en su eslabón crítico (aplicación): el trabajo de la experta dejó de estar muerto en una tabla.

- **Cómo cerró**: el resolver del diccionario (`_match_by_argentino_dict`) ahora resuelve por código, con longest-match para desambiguar variantes (un hallazgo de la carga a escala: "vigilador/a de personal" 3122 ya no cae en "vigilador/a" 5414). 0 regresiones en el TEST, 2 ganancias.
- **Validación de dominio como verdad**: las denominaciones se cargaron tal cual las validó Cyn, sin re-filtrar. El dry-run de blast-radius (método permanente) cazó la única sobre-generalización real ("lobos", una localidad), distinguiéndola del alto-blast legítimo (denominaciones de cobertura amplia validadas por Cyn). El caso semilla "intendente de obra" cerró el arco completo: error automático (conserje) → cazado por dominio → corregido por Cyn a capataz (3123.1.1).
- **Métrica**: regresión en el TEST, NO generalización (las denominaciones de Cyn no recurren entre ofertas — son casi únicas; el valor real se ve en producción cuando el término reaparece). De 50 denominaciones con código, el sistema erraba en ~31 (62%) — confirma que el vocabulario argentino corrige donde el sistema más falla.
- **Alcance honesto**: G3 cerró la vía de denominación (Eje 3, vocabulario). El loop sigue abierto para las otras vías (G1 ocupación, G6 skills…) y las 166 ofertas pisadas por reglas siguen dependiendo del Eje 4. Es el primer ladrillo, no el loop completo.

**P-01 reubicada (no eliminada)**: la derivación inversa `LIMIT 1` ("café verde", label arbitrario silencioso) murió en el diccionario (Paso 0: fallo ruidoso en vez de label arbitrario; P.3 había probado 0/3839 la usaban). El riesgo de resolución-por-label silenciosa **migró a su única ubicación viva**: el fallback label-LIKE del canal de reglas (`_resolve_rule_target`, `match_ofertas_v3.py:1314-1318`). Es deuda del Eje 4 — ítem de arranque junto a los 104 override-duro.

**Nuevo activo — taxonomía de contexto de Cyn (insumo del Eje 4)**: `exports/cyn_backlog/taxonomia_contexto_cyn.md`. Al validar G3, Cyn entregó algo más que denominaciones: para 6 familias ambiguas (operario, técnico, arquitecto, pintor, editor, herrero) + 2 del GRUPO B, escribió **árboles de desambiguación** — las reglas de cuándo cada denominación va a cada código según las tareas del aviso. Es la taxonomía ocupacional argentina escrita por la experta, y es la **especificación de las reglas de contexto del Eje 4**. Cuando se ataquen las reglas, estos árboles son el insumo experto (junto a los 104 override-duro y el fallback label-LIKE). Estos casos NO van al diccionario plano (son contexto-dependientes; cargar "operario de producción" plano rompería sus 8 variantes).

**Deuda registrada — bug de limpieza de título NLP**: 5 títulos llegaron con ruido sin limpiar (números de referencia, IDs, códigos de búsqueda) y el sistema clasificó sobre basura. Bug del pre-procesamiento de título, independiente del diccionario. Registrado en `docs/issues/`.
...
- **F0.7 + G3 — Lenguaje de Cyn + primer cierre de loop** (`SPEC_S1C_F07_*`, PR #44; G3 carga, PR #46 mergeado 2026-06-30): F0.7 clasificó el lenguaje de Cyn en 11 categorías y fijó el train/test split. G3 cerró el loop en la vía de denominación: **46 denominaciones validadas por Cyn clasifican ofertas** vía diccionario resolviendo por `esco_code`; 0 regresiones / 2 ganancias en el TEST; caso semilla "intendente de obra" corregido a capataz. **Primer cierre de loop del proyecto** — P-14 parcialmente resuelto (ver Eje 2). Subproductos: P-01 reubicada a deuda de reglas (Eje 4); taxonomía de contexto de Cyn (`taxonomia_contexto_cyn.md`) como insumo del Eje 4; bug NLP de limpieza de título registrado.
