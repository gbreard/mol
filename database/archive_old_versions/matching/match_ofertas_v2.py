...
def nivel_1_5_reglas_negocio(
    ...
    # R13: Enfermero/Enfermera
    r13 = reglas_forzar.get("R13_enfermero", {})
    if r13.get("activa", False):
        enfermero_terms = r13.get("condicion", {}).get("titulo_contiene_alguno",
            ["enfermero", "enfermera", "enfermeria", "nurse"])
        if _texto_contiene_alguno(titulo, enfermero_terms):
            logger.info(f"Regla R13 aplicada: Enfermero/Enfermera detectado")
            isco = r13.get("accion", {}).get("forzar_isco", "2221")
            label = r13.get("accion", {}).get("esco_label", "Enfermero/enfermera")
            return _crear_resultado_regla("R13_enfermero", isco, label, db_conn)

    # R14: Contador/Auditor
    r14 = reglas_forzar.get("R14_contador_auditor", {})
    if r14.get("activa", False):
        contador_terms = r14.get("condicion", {}).get("titulo_contiene_alguno",
            ["contador", "contadora", "contable", "auditor", "auditoria"])
        if _texto_contiene_alguno(titulo, contador_terms):
            logger.info(f"Regla R14 aplicada: Contador/Auditor detectado")
            isco = r14.get("accion", {}).get("forzar_isco_familia", "241") + "0"
            label = r14.get("accion", {}).get("esco_label", "Contador/contadora")
            return _crear_resultado_regla("R14_contador_auditor", isco, label, db_conn)

    # R15: Customer Care/Atencion al Cliente
    r15 = reglas_forzar.get("R15_customer_care", {})
    if r15.get("activa", False):
        cc_terms = r15.get("condicion", {}).get("titulo_contiene_alguno",
            ["customer care", "customer service", "atencion al cliente", "call center"])
        if _texto_contiene_alguno(titulo, cc_terms):
            logger.info(f"Regla R15 aplicada: Customer Care detectado")
            isco = r15.get("accion", {}).get("forzar_isco", "4222")
            label = r15.get("accion", {}).get("esco_label", "Telefonista de centro de contacto")
            return _crear_resultado_regla("R15_customer_care", isco, label, db_conn)

    # R16: Barista/Heladero/Cafeteria
    r16 = reglas_forzar.get("R16_barista_heladero", {})
    if r16.get("activa", False):
        barista_terms = r16.get("condicion", {}).get("titulo_contiene_alguno",
            ["barista", "heladero", "heladera", "heladeria", "cafeteria"])
        if _texto_contiene_alguno(titulo, barista_terms):
            logger.info(f"Regla R16 aplicada: Barista/Heladero detectado")
            isco = r16.get("accion", {}).get("forzar_isco", "5132")
            label = r16.get("accion", {}).get("esco_label", "Barista")
            return _crear_resultado_regla("R16_barista_heladero", isco, label, db_conn)

    # R17: Compliance/Legal
    r17 = reglas_forzar.get("R17_compliance_legal", {})
    if r17.get("activa", False):
        legal_terms = r17.get("condicion", {}).get("titulo_contiene_alguno",
            ["compliance", "legales", "legal", "abogado", "abogada", "juridico"])
        if _texto_contiene_alguno(titulo, legal_terms):
            logger.info(f"Regla R17 aplicada: Compliance/Legal detectado")
            isco = r17.get("accion", {}).get("forzar_isco", "2611")
            label = r17.get("accion", {}).get("esco_label", "Abogado/abogada")
            return _crear_resultado_regla("R17_compliance_legal", isco, label, db_conn)

    # R18: Payroll/Liquidacion de Sueldos
    r18 = reglas_forzar.get("R18_payroll_nominas", {})
    if r18.get("activa", False):
        payroll_terms = r18.get("condicion", {}).get("titulo_contiene_alguno",
            ["liquidacion de sueldos", "payroll", "nominas", "sueldos y jornales"])
        if _texto_contiene_alguno(titulo, payroll_terms):
            logger.info(f"Regla R18 aplicada: Payroll/Nominas detectado")
            isco = r18.get("accion", {}).get("forzar_isco", "4313")
            label = r18.get("accion", {}).get("esco_label", "Empleado de nominas")
            return _crear_resultado_regla("R18_payroll_nominas", isco, label, db_conn)

    # R19: Project Manager
    r19 = reglas_forzar.get("R19_project_manager", {})
    if r19.get("activa", False):
        pm_terms = r19.get("condicion", {}).get("titulo_contiene_alguno",
            ["project manager", "jefe de proyecto", "gerente de proyecto", "lider de proyecto"])
        if _texto_contiene_alguno(titulo, pm_terms):
            logger.info(f"Regla R19 aplicada: Project Manager detectado")
            isco = r19.get("accion", {}).get("forzar_isco", "1213")
            label = r19.get("accion", {}).get("esco_label", "Director de proyectos")
            return _crear_resultado_regla("R19_project_manager", isco, label, db_conn)

    # R20: Vendedor Ferreteria/Mostrador
    r20 = reglas_forzar.get("R20_vendedor_ferreteria", {})
    if r20.get("activa", False):
        ferret_terms = r20.get("condicion", {}).get("titulo_contiene_alguno",
            ["venta mostrador", "vendedor mostrador", "casa de electricidad", "ferreteria"])
        if _texto_contiene_alguno(titulo, ferret_terms):
            logger.info(f"Regla R20 aplicada: Vendedor Ferreteria/Mostrador detectado")
            isco = r20.get("accion", {}).get("forzar_isco", "5223")
            label = r20.get("accion", {}).get("esco_label", "Vendedor especializado en ferreteria y pintura")
            return _crear_resultado_regla("R20_vendedor_ferreteria", isco, label, db_conn)

    # R21: Lider/Supervisor Atencion al Cliente
    r21 = reglas_forzar.get("R21_lider_atencion_cliente", {})
    if r21.get("activa", False):
        lider_terms = r21.get("condicion", {}).get("titulo_contiene_alguno",
            ["lider atencion", "supervisor atencion", "coordinador atencion", "jefe atencion"])
        if _texto_contiene_alguno(titulo, lider_terms):
            logger.info(f"Regla R21 aplicada: Lider Atencion al Cliente detectado")
            isco = r21.get("accion", {}).get("forzar_isco", "4222")
            label = r21.get("accion", {}).get("esco_label", "Supervisor de centro de contacto")
            return _crear_resultado_regla("R21_lider_atencion_cliente", isco, label, db_conn)

    # R22: Coordinador/Tecnico Instalaciones
    r22 = reglas_forzar.get("R22_instalaciones_mantenimiento", {})
    if r22.get("activa", False):
        instal_terms = r22.get("condicion", {}).get("titulo_contiene_alguno",
            ["coordinador instalaciones", "tecnico instalaciones", "mantenimiento edilicio", "facility"])
        if _texto_contiene_alguno(titulo, instal_terms):
            logger.info(f"Regla R22 aplicada: Instalaciones/Mantenimiento detectado")
            isco = r22.get("accion", {}).get("forzar_isco", "7126")
            label = r22.get("accion", {}).get("esco_label", "Tecnico de mantenimiento de instalaciones")
            return _crear_resultado_regla("R22_instalaciones_mantenimiento", isco, label, db_conn)
