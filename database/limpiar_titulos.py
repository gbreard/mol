...
def limpiar_titulo(titulo: str, config: Dict[str, Any] = None) -> str:
    ...
    # Aplicar limpieza en loop hasta que no haya cambios (max 5 iteraciones)
    for _ in range(5):
        titulo_antes = titulo
        ...
        # 5.5 Eliminar sufijos empresariales completos (patrones)
        for patron_info in config.get("sufijos_empresariales", {}).get("patrones", []):
            patron = patron_info.get("patron", "")
            if patron:
                titulo = re.sub(patron, '', titulo, flags=re.IGNORECASE)

        # 5.6 Eliminar localidades al final del titulo
        localidades = config.get("localidades_final", {}).get("lista", [])
        if localidades:
            # Ordenar por longitud descendente para evitar matchear parciales
            localidades_ordenadas = sorted(localidades, key=len, reverse=True)
            for loc in localidades_ordenadas:
                # Con guion antes
                titulo = re.sub(rf'\s*-\s*{re.escape(loc)}$', '', titulo, flags=re.IGNORECASE)
                # Sin guion, al final
                titulo = re.sub(rf'\s+{re.escape(loc)}$', '', titulo, flags=re.IGNORECASE)
