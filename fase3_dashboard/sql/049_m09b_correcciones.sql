-- Migration 049: M-09b — Circulación de correcciones expertas
-- 1. Campo procesada_en_pipeline en ofertas_dashboard
-- 2. RPC get_correcciones_sin_procesar()
-- 3. Alerta en get_pipeline_status()

-- 1. Campo para trackear si una corrección fue procesada
ALTER TABLE ofertas_dashboard
ADD COLUMN IF NOT EXISTS procesada_en_pipeline TIMESTAMPTZ;

-- 2. RPC: contar correcciones sin procesar
CREATE OR REPLACE FUNCTION get_correcciones_sin_procesar()
RETURNS TABLE (
    total INT,
    con_isco INT,
    con_nota INT,
...
AS $$
    SELECT
        COUNT(*)::INT as total,
        COUNT(*) FILTER (
            WHERE validacion_correcciones->>'ocupacion_corregida' IS NOT NULL
        )::INT as con_isco,
        COUNT(*) FILTER (
            WHERE validacion_correcciones->>'nota' IS NOT NULL
            AND LENGTH(validacion_correcciones->>'nota') > 50
        )::INT as con_nota,
        COUNT(*) FILTER (
            WHERE validacion_correcciones->>'skills_editadas' IS NOT NULL
        )::INT as con_skills,
        MIN(validacion_humana_at) as desde
    FROM ofertas_dashboard
    WHERE validacion_correcciones IS NOT NULL
    AND (procesada_en_pipeline IS NULL
         OR validacion_humana_at > procesada_en_pipeline);
$$;

-- 3. Actualizar get_pipeline_status() con alerta de correcciones
...
DECLARE
  ...
  v_correcciones_pendientes bigint;
  v_alertas json;
BEGIN
  ...
  -- M-09b: Correcciones de validadores sin procesar
  SELECT COUNT(*) INTO v_correcciones_pendientes
  FROM ofertas_dashboard
  WHERE validacion_correcciones IS NOT NULL
    AND (procesada_en_pipeline IS NULL
         OR validacion_humana_at > procesada_en_pipeline);
  ...
  FROM (
    ...
    -- M-09b: Correcciones de validadores sin procesar
    SELECT 8, 'info'::text,
      v_correcciones_pendientes || ' correcciones de validadores sin procesar',
      'ver_correcciones_pendientes',
      'Hay correcciones expertas de validadores que no fueron incorporadas al sistema. Revisá las correcciones pendientes y generá un reporte para procesarlas.'::text
    WHERE v_correcciones_pendientes > 0
    ...
    WHERE v_estado.fase1_dias_desde_scraping <= 3
      ...
      AND v_correcciones_pendientes = 0
  ) a
