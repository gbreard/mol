-- Migration 071: get_pipeline_status() — COUNT acotado de auto-issues
-- ============================================================
-- Contexto (issue 2026-09-24): la pantalla de KPIs (admin/metricas) tira 57014.
-- Causa: el 2º COUNT contaba ~1,6M filas del auto-validator en `issues` en cada
-- carga (seq scan; `issues(estado)` no discrimina, 93% es auto-pendiente) → ~5s,
-- clava el statement_timeout de 5s.
--
-- Fix (opción b, decidida por Gerardo): el panel solo necesita "hay backlog", NO
-- el número exacto. Se acota el COUNT con LIMIT → el scan frena apenas junta el
-- tope, sin recorrer la tabla entera. Idéntico a 041 salvo el bloque marcado.
--
-- Semántica: `issues_auto_pendientes` ahora es min(count, 500). El front puede
-- renderizarlo como "500+" cuando llega al tope. NO alimenta alertas ni CASE
-- (solo va a resumen.issues_auto_pendientes), así que el cambio es cosmético.
--
-- NOTA: esto NO reemplaza el arreglo del emisor (D1 gate --skip-issues + D2 dedup
-- truncada a 1000 en sync_to_supabase.py) ni la limpieza del backlog. Solo saca
-- el 57014 de la pantalla. Ver docs/issues/2026-09-24_timeout_rpcs_admin_metricas.md.
--
...
BEGIN
  ...
  -- Issues humanos pendientes: EXACTO (subset chico ~cientos, usa el índice
  -- parcial idx_issues_humanos_created_at). Es el número accionable del panel.
  SELECT COUNT(*) INTO v_issues_humanos
  FROM issues
  WHERE estado = 'pendiente'
    AND autor_email != 'auto-validator@mol.gob.ar';

  -- >>> CAMBIO 071: auto-issues pendientes ACOTADO (evita seq scan de ~1,6M).
  -- LIMIT frena el scan al juntar el tope; el valor es min(count, 500).
