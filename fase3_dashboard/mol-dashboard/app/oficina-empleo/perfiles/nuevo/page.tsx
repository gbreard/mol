...
import { useSkillCapture } from '@/components/oficina-empleo/useSkillCapture'
...
export default function NuevoPerfilPage() {
  ...
  async function handleSave() {
    ...
    setSaving(true)
    try {
      // 1. Create or find persona (upsert by DNI)
      const personaRes = await fetch('/api/personas', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ nombre: capture.nombre, dni: capture.dni }),
      })
      if (!personaRes.ok) {
      ...
      // 2. Create perfil linked to persona
      ...
      <div className="max-w-6xl mx-auto px-4 py-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 items-start">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
            <h2 className="text-lg font-semibold text-gray-900">Captura de skills</h2>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5 lg:sticky lg:top-6" style={{ maxHeight: 'calc(100vh - 120px)' }}>
            <h2 className="text-lg font-semibold text-gray-900 mb-3">Perfil acumulado</h2>
            <SkillProfilePanel
              ocupaciones={capture.ocupaciones}
