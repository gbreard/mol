"""
Explorador Técnico de ComputRabajo
===================================

Script para analizar la arquitectura técnica de ar.computrabajo.com
...
class ComputRabajoExplorer:
    ...
    def on_request(self, request):
        ...
        if '/api/' in request.url or request.resource_type in ['xhr', 'fetch']:
            ...
            if '/ofertas' in request.url or '/empleos' in request.url or '/jobs' in request.url:
                print(f"\n[API CALL] {request.method} {request.url}")
                print(f"[HEADERS]")
                for key, value in request.headers.items():
                    if key.lower().startswith('x-') or key.lower() in ['authorization', 'cookie']:
    ...
    def explore(self, url="https://ar.computrabajo.com/", wait_time=10):
        """Explora ComputRabajo y analiza su arquitectura"""

        print("=" * 70)
        print("EXPLORADOR TECNICO DE COMPUTRABAJO")
        print("=" * 70)
        print(f"\nURL a explorar: {url}")
        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                print(f"WARNING: Error en scroll: {e}")
    ...
    def _analyze_results(self, html_content):
        ...
        job_patterns = [
            ...
            r'data-job-id',
            r'data-aviso-id',
            r'class="[^"]*bRS[^"]*"',  # ComputRabajo usa esto
            r'id="p\d+"',  # IDs de puestos
        ]
        ...
        if not techs:
            print("   - JavaScript vanilla / Server-side rendering")
    ...
    def _save_analysis(self):
        ...
        filename = f"../data/raw/computrabajo_analysis_{self.timestamp}.json"
        with open(filename, 'w', encoding='utf-8') as f:
